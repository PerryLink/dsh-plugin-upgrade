...
- Pin the `@deepseek-ai/dsh-*` dev dependency to the published `0.1.5-rc.2` line; the monthly Compat workflow now runs against `0.1.5-rc.2`. The card itself stays version-locked on its corridor (a new corridor is a new package), so no card text changes.
