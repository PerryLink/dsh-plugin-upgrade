...
const CARD = 'references/v0.1.5-alpha.1-to-v0.1.5-rc.1.md'
...
test('registers the packaged skill on the real skills service and removes it on dispose', async () => {
  ...
  assert.ok(list.some(s => s.name === 'plugin-upgrade-015rc1'), `expected plugin-upgrade-015rc1 in ${list.map(s => s.name).join(',')}`)
  const def = await ctx.skills.get('plugin-upgrade-015rc1')
  ...
  assert.equal(path.basename(def.resourceBase.path), 'plugin-upgrade-015rc1')
  assert.ok(existsSync(path.join(def.resourceBase.path, CARD)))
  await fiber.dispose()
  const after = await ctx.skills.list()
  assert.ok(!after.some(s => s.name === 'plugin-upgrade-015rc1'), 'skill must disappear after dispose')
})
...
test('enabled: false mounts without registering anything', async () => {
  ...
  const list = await ctx.skills.list()
  assert.ok(!list.some(s => s.name === 'plugin-upgrade-015rc1'))
})
...
test('an empty skill body fails loud', async () => {
  const tmp = mkdtempSync(path.join(tmpdir(), 'dshuprc1-empty-'))
  try {
    const dir = path.join(tmp, 'plugin-upgrade-015rc1')
    mkdirSync(dir, { recursive: true })
    writeFileSync(path.join(dir, 'SKILL.md'), '---\nname: plugin-upgrade-015rc1\n---\n\n')
    assert.throws(() => plugin.readSkillBundle(tmp, 'plugin-upgrade-015rc1'), /skill body is empty/)
  } finally {
    rmSync(tmp, { recursive: true, force: true })
  }
})

test('a frontmatter without a name fails loud', async () => {
  const tmp = mkdtempSync(path.join(tmpdir(), 'dshuprc1-noname-'))
  try {
    const dir = path.join(tmp, 'plugin-upgrade-015rc1')
    mkdirSync(dir, { recursive: true })
    writeFileSync(path.join(dir, 'SKILL.md'), '---\ndescription: no name here\n---\nbody\n')
    assert.throws(() => plugin.readSkillBundle(tmp, 'plugin-upgrade-015rc1'), /missing a name/)
  } finally {
    rmSync(tmp, { recursive: true, force: true })
  }
})
...
test('readSkillBundle mounts a CRLF-converted bundle', () => {
  const tmp = mkdtempSync(path.join(tmpdir(), 'dshuprc1-crlf-'))
  try {
    const dir = path.join(tmp, 'plugin-upgrade-015rc1')
    mkdirSync(dir, { recursive: true })
    const crlf = readFileSync(path.join(root, 'skills', 'plugin-upgrade-015rc1', 'SKILL.md'), 'utf8').replace(/\r?\n/g, '\r\n')
    writeFileSync(path.join(dir, 'SKILL.md'), crlf)
    const bundle = plugin.readSkillBundle(tmp, 'plugin-upgrade-015rc1')
    assert.equal(bundle.frontmatterName, 'plugin-upgrade-015rc1')
